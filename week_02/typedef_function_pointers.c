...
/* dev_op     — the *function* type every operation must have.
 * dev_op_fn  — a pointer to such a function; what the table stores. */
typedef int dev_op(void *ctx, uint8_t *buf, size_t len);
typedef dev_op *dev_op_fn;

/* Declaring through the typedef means the signature is stated once.
 * A definition that drifts from it no longer compiles. */
static dev_op op_read;
static dev_op op_write;
static dev_op op_erase;

/* const applies to the pointer, so this is the same type as
 * int (*const ops[OP_COUNT])(void *, uint8_t *, size_t). */
static const dev_op_fn ops[OP_COUNT] = {
    [OP_READ] = op_read,
    [OP_WRITE] = op_write,
    [OP_ERASE] = op_erase,
};
...
int main(void)
{
  ...
  dev_op_fn erase = ops[OP_ERASE]; /* the typedef also tidies locals */
  int rc;
